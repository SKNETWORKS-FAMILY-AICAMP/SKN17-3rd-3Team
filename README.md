...
벡터DB, rag, 파인튜닝이 추가되지 않은 sllm 모델의 응답과 비교해 정확한 정보와 맥락에 맞는 정보를 주는 모델을 만드는 것입니다.
따라서 "부모님과 함께 부산 여행에 간다면 어떤 코스를 추천해줄래?", "남자친구랑 함께 제주도 여행을 처음 가는데 어디에 가야할까?"라는 질문을 각각 입력하여 답변을 비교하였습니다.
결과적으로 ---
